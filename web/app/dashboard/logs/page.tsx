  ...
  const fetchLiveLogs = useCallback(async () => {
    ...
    try {
      const [runsRes, intentsRes] = await Promise.all([
        fetch(`/api/runs?limit=20${activeRepo ? `&repo=${encodeURIComponent(activeRepo)}` : ""}`).catch(() => null),
      ...
      if (runsRes && runsRes.ok) {
        ...
        for (const run of runsList) {
          ...
          if (Array.isArray(run.test_results)) {
            for (const step of run.test_results) {
              entries.push({
                id: `step-${step.id || Math.random()}`,
                time: timeStr,
            ...
            <div className="space-y-1.5 text-[11px] text-slate-600 font-mono">
              <div className="flex items-center justify-between py-0.5">
                ...
                <span className="truncate max-w-[150px] font-medium text-slate-800">
                  {activeRepo ? activeRepo.split("/")[1] || activeRepo : "All Projects"}
                </span>
              </div>
              <div className="flex items-center justify-between py-0.5">
